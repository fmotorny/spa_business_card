...
export class ApiService {
  ...
  public getPageDataByUrl(url: string): Observable<CollectionResModel> {
    return this.http.get<CollectionResModel>(
      environment.apiUrl + `pages?filters[url][$eq]=${url}`,
    );
  }

  public sendMessToTG(feedbackData: any): Observable<ResponseModel> {
    return this.http.post<ResponseModel>(
      environment.wellicanApi + `send-msg`,
      feedbackData,
    );
  }
}
